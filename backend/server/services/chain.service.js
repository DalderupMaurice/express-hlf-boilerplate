...
export default class ChaincodeService {
  ...
  prepareRequest = async (user, functionName, funcArgs = [""], invocation = true) =>
  ...
  query = async request =>
    new Promise(async (resolve, reject) => {
      const queryResult = await this.channel.queryByChaincode(request).catch(err => reject(err));
  ...
  invoke = request =>
    new Promise(async (resolve, reject) => {
      const proposalResult = await this.channel.sendTransactionProposal(request).catch(err => reject(err));
      ...
      this.eventHub.registerTxEvent(
        ...
        (tx, statusCode) => {
          ...
          Logger(labels.TRANSACTION).info(
            `The transaction has been committed on peer ${this.eventHub._ep._endpoint.addr}`
          );
